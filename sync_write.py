...
DXL_MAXIMUM_POSITION_VALUE  = 650            # and this value (note that the Dynamixel would not move when the position value is out of movable range. Check e-manual about the range of the Dynamixel you use.)
...
for i in range(1,12+1):
    dxl_comm_result, dxl_error = packetHandler.write1ByteTxRx(portHandler, i, ADDR_AX_TORQUE_ENABLE, TORQUE_ENABLE)
    ...
    dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(portHandler, i, ADDR_AX_TORQUE_LIMIT, 1023)
    if dxl_comm_result != COMM_SUCCESS:
        print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
    elif dxl_error != 0:
        print("%s" % packetHandler.getRxPacketError(dxl_error))
    else:
        print("Dynamixel#%d change torque limit" % i)


    dxl_comm_result, dxl_error =  packetHandler.write2ByteTxRx(portHandler, i, ADDR_AX_MOVING_SPEED, 128)
    if dxl_comm_result != COMM_SUCCESS:
        print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
    elif dxl_error != 0:
        print("%s" % packetHandler.getRxPacketError(dxl_error))
    else:
        print("Dynamixel#%d change moving speed limit" % i)
...
while 1:
    ...
    param_goal_position = [DXL_LOBYTE(DXL_LOWORD(dxl_goal_position[index])), DXL_HIBYTE(DXL_LOWORD(dxl_goal_position[index]))]
    ...
    while 1:
        ...
        dxl1_present_position, dxl_comm_result, dxl_error = packetHandler.read4ByteTxRx(portHandler, 11, ADDR_AX_PRESENT_POSITION)
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % packetHandler.getRxPacketError(dxl_error))
        ...
        dxl2_present_position, dxl_comm_result, dxl_error = packetHandler.read4ByteTxRx(portHandler, 12, ADDR_AX_PRESENT_POSITION)
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % packetHandler.getRxPacketError(dxl_error))
...
for i in range(1,12+1):
    packetHandler.write1ByteTxRx(portHandler, i, ADDR_AX_TORQUE_ENABLE, TORQUE_DISABLE)
